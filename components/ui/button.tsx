...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center gap-2 border border-transparent bg-clip-padding text-base font-bold whitespace-nowrap transition-[transform,background,opacity,color,box-shadow] duration-150 outline-none select-none focus-visible:outline-2 focus-visible:outline-ink focus-visible:outline-offset-2 active:enabled:scale-[0.97] disabled:pointer-events-none disabled:opacity-55 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-[18px]",
  {
    variants: {
      variant: {
        default:
          "bg-lime text-ink hover:enabled:bg-lime-hover",
        primary:
          "bg-lime text-ink hover:enabled:bg-lime-hover",
        outline:
      ...
      size: {
        ...
        sm: "min-h-9 w-auto gap-1.5 rounded-pill px-3.5 py-2 text-[0.85rem] [&_svg:not([class*='size-'])]:size-[15px]",
